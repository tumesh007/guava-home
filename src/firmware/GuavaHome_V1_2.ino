/*
 * ============================================================================
 * PROJECT  : Guava Home — Smart Home Controller V1.2
 * BOARD    : ESP32 DevKit V1 (38-pin)
 * PARTITION: Default 4MB with spiffs
 *
 * V1.2 UPDATES:
 * - Added a Telegram Alert Preferences checklist to the Web UI.
 * - Allows granular control over which automated events trigger a Telegram push.
 * ============================================================================
 */
...
// ═══════════════════════════════════════════════════════════════════════
#define PIN_DHT         4
#define PIN_TRIG       12   // HC-SR04 TRIG — idles LOW (safe strapping pin)
#define PIN_ECHO       13   // HC-SR04 ECHO — 5V via 1kΩ/2kΩ divider
#define PIN_BUZZER     14
#define PIN_RELAY1     15   // Fan
#define PIN_RELAY2     16   // Light
#define PIN_RFID_RST   17
#define PIN_RFID_SS     5
// SPI: SCK=18, MISO=19, MOSI=23  |  I2C: SDA=21, SCL=22
#define PIN_RELAY3     25   // Door lock
#define PIN_BTN1       26   // Exit button — INPUT_PULLUP, LOW=pressed
#define PIN_PIR        27
#define PIN_SOIL       32
#define PIN_RELAY4     33   // Water pump
#define PIN_LDR        34   // Input-only ADC1
#define PIN_BTN2       35   // LCD page button — 10kΩ pull-DOWN, HIGH=pressed
#define PIN_MQ2        36   // MQ-2 gas — input-only ADC1, via divider
#define PIN_CLAP       39   // KY-037 sound sensor DO — input-only, ISR FALLING
...
// ═══════════════════════════════════════════════════════════════════════
// DYNAMIC CONFIG — all loaded from flash, all editable from web UI
// ═══════════════════════════════════════════════════════════════════════
String        cfg_ssid;
String        cfg_pass;
String        cfg_tgToken;
...
// ═══════════════════════════════════════════════════════════════════════
// RELAY CONTROL — all ACTIVE LOW (LOW = relay ON)
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// ULTRASONIC — TRIG=GPIO12 ECHO=GPIO13
// ═══════════════════════════════════════════════════════════════════════
long getDistance() {
...
// ═══════════════════════════════════════════════════════════════════════
// TELEGRAM SEND — null-safe
// ═══════════════════════════════════════════════════════════════════════
void tgSend(const String& msg) {
  if (apMode || otaActive) return;
  ...
  switch (lcdPage) {
    ...
    case PAGE_MAIN:
      if (!isnan(sens.temp) && sens.temp > 0)
        ...
        snprintf(line1, 17, "DHT starting... ");
    ...
    case PAGE_SENSORS:
      ...
      lcd.setCursor(0, 0); lcd.print(line1);
      snprintf(line2, 17, "Gas:%-4d D:%-3ld%s",
        sens.gas, sens.dist == 999 ? 0 : sens.dist,
        (sens.gas > THRESH_GAS_WARN) ? "!" : " ");
      lcd.setCursor(0, 1); lcd.print(line2);
      break;

    case PAGE_DEVICES:
      snprintf(line1, 17, "Fan:%-3s Lgt:%-3s ",
        dev.fan ? "ON" : "OFF", dev.light ? "ON" : "OFF");
      lcd.setCursor(0, 0); lcd.print(line1);
      snprintf(line2, 17, "Door:%-4s Pmp:%-3s",
        dev.door ? "OPEN" : "LOCK", dev.pump ? "ON" : "OFF");
      lcd.setCursor(0, 1); lcd.print(line2);
      break;

    case PAGE_NETWORK:
      if (apMode) {
        lcd.setCursor(0, 0); lcd.print("AP: Guava_Setup ");
      } else {
        lcd.setCursor(0, 0);
...
String buildJSON() {
  String logStr = "";
  int count = min((int)evIdx, LOG_SIZE);
  for (int i = 0; i < count; i++) {
  ...
  doc["sol"] = sens.soil;
  doc["gas"] = sens.gas;
  doc["gw"]  = (sens.gas > THRESH_GAS_WARN);
  doc["dst"] = sens.dist;
  doc["clp"] = clapCount;
  doc["f"]   = dev.fan;
  doc["l"]   = dev.light;
...
void executeCmd(String cmd, int src = 1) {
  ...
  if      (cmd == "fan_on")    { setFan(true);    logEvent("Fan ON");    tgSend("Fan ON");    }
    ...
    String dhtStr = (!isnan(sens.temp) && sens.temp > 0)
      ? String(sens.temp, 1) + "C / " + String(sens.humidity, 0) + "%" : "DHT not ready";
    String soilStr = String(sens.soil) +
      (sens.soil > cfg_soilDryThresh ? " DRY" : sens.soil < THRESH_SOIL_WET ? " WET" : " OK");
    String msg =
      "Guava Home V1.2\n"
      ...
      "Auto-status: " + (cfg_tgInterval > 0 ? String(cfg_tgInterval) + " min" : "off") + "\n"
      "OTA: ready (GuavaHome)";
    if (src == 0) tgSend(msg);
...
<div id='tab2' class='tab'>
  ...
  <p class='hint'>Higher = triggers alert at drier soil. Lower for plants needing more water (e.g. 2000). Current: <span id='curSDT'>3000</span></p>
...
void setup() {
  ...
  lcd.print("Guava Home"); lcd.setCursor(0,1); lcd.print("V1.2 Booting...");
  ...
  // Load alert preferences (default true if not set)
  ...
  Serial.println("[CFG] Temp thresh: " + String(cfg_tempThresh) + "C");
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println(">>> Dashboard: http://" + ip);
    Serial.println(">>> OTA page:  http://" + ip + "/update");
    ...
    ArduinoOTA.begin();
    Serial.println("[OTA] Ready — hostname: GuavaHome");

    lcd.clear(); lcd.print("WiFi OK"); lcd.setCursor(0,1); lcd.print(ip.c_str());
    delay(2000);
    tgSend("Guava Home V1.2 Online\nDashboard: http://" + ip);
    ...
    Serial.println("[AP] Guava_Setup → 192.168.4.1");
...
void loop() {
  ...
  if (dev.door && (now - tDoor >= DOOR_RELOCK_MS)) lockDoor();
  ...
  if (!dev.door && rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
    ...
    lastScannedUID = uid;
    Serial.println("[RFID] " + uid + (name.length() ? " = "+name : " = UNKNOWN"));
    if (name.length() > 0) {
      logEvent("RFID: " + name); 
  ...
  if (now - tFast > SENSOR_FAST_MS) {
    ...
    if (sens.dist > 0 && sens.dist <= LCD_WAKE_DIST_CM) {
      lcd.backlight(); tLCDWake = now;
    } else if (now - tLCDWake > LCD_SLEEP_MS) {
      lcd.noBacklight();
    }
  }
  ...
  if (pump.enabled) {
    if (!pump.phaseOn) {
      if (now - pump.phaseStart >= (cfg_pumpOffTime * 1000UL)) {
        ...
        logEvent("Pump ON (cycle " + String(pump.cycleCount) + ")");
  ...
  if (!isnan(sens.temp) && sens.temp > 0 && sens.temp >= cfg_tempThresh && !dev.fan) {
    setFan(true); logEvent("Fan auto ON: " + String(sens.temp,1) + "C");
